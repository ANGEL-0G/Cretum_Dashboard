...
const XLATE_PATTERNS = [
  ...
  [/^Día (\d+) \((.+)\)$/, 'Day $1 ($2)'],
  [/^1er día de venta legal.*$/, '1st legal sale date (direct: Aug 6, 2026 · funds: Sep 9, 2026)'],
...
const XLATE_LONG = [
  ...
  ['Liberación escalonada dentro de la ventana de 180 días (directos venden desde el 6 ago 2026; vehículos de fondo desde el 9 sep 2026). Expira por completo el 8 dic 2026.',
...
const XP_CO_FICHAS = {
  ...
  'Groq':             { domain: 'groq.com',             category: 'Semiconductores · IA', stage: 'Adquirida (Nvidia)', tagline: 'Chips de inferencia de IA (LPU) ultrarrápidos. En liquidación vía transacción con Nvidia; distribuciones esperadas de al menos $73/acción en 2026.' , en: { category: 'Semiconductors · AI', stage: 'Acquired (Nvidia)', tagline: 'Ultra-fast AI inference chips (LPU). Being liquidated via the Nvidia transaction; expected total distributions of at least $73/share in 2026.' } },
  ...
  try {
    ...
    try {
      ...
      if (co27.length) {
        ...
        let sA = 0, sB = 0, sAd = 0, sAf = 0, sBd = 0, sBf = 0;
        co27.forEach(p => {
          const anc = spxAnchor(p.series?.name || '');
          spxTranches(p.series?.name || '').forEach(t => {
            const sh = (+p.shares) * t.portion;
            if (t.structure === 'A') { sA += sh; if (anc === 'dir') sAd += sh; else sAf += sh; }
            else { sB += sh; if (anc === 'dir') sBd += sh; else sBf += sh; }
          });
        });
        const today = new Date().toISOString().slice(0, 10);
        const pctNum = s => s === 'Remanente' ? 17 : (parseFloat(s) || 0);
        const ev = [];
        SPX_LOCKUP_B_DIR.forEach(e => { if (e.date >= today) ev.push({ ...e, sh: (pctNum(e.pct) / 100) * (sBd + sAd / 2) }); });
        SPX_LOCKUP_B_FONDO.forEach(e => { if (e.date >= today) ev.push({ ...e, sh: (pctNum(e.pct) / 100) * (sBf + sAf / 2) }); });
        SPX_LOCKUP_A_EXT.forEach(e => { if (e.date >= today) ev.push({ ...e, sh: (pctNum(e.pct) / 100) * (sA / 2) }); });
        // fusionar hitos idénticos de ambas anclas (ej. Día 180, 8-dic ambos)
...
// Calendario con FECHAS REALES (doc oficial "SpaceX distribution: lockup summary by vehicle",
// 29-jul-2026; ver invariante spacex-lockup-fechas-reales-2026-08-05). Dos anclas del primer
// tramo según el ORIGEN del vehículo: DIRECTOS (22F/22J/22K) venden desde el 6-ago-2026;
// DE FONDO (Fund IV/V, 26B, 26B QC y resto) desde el 9-sep-2026. Día 180 = 8-dic-2026 para
...
const SPX_LOCKUP_B_FONDO = [
  ...
  { date: '2026-09-24', pct: '7%',  label: 'Bloque 1 (~15 días)' },
  { date: '2026-10-09', pct: '7%',  label: 'Bloque 2' },
  { date: '2026-10-24', pct: '7%',  label: 'Bloque 3' },
  { date: '2026-11-08', pct: '7%',  label: 'Bloque 4' },
  { date: '2026-11-23', pct: '7%',  label: 'Bloque 5' },
  { date: '2026-12-01', pct: '28%', label: '2º cliff (~fin nov/dic 2026)' },
...
const SPX_STRUCTURES = {
  B: {
    ...
    phases: [
      ...
      { hito: 'Bloque 1 (directos: 20 ago 2026 · fondos: ~24 sep 2026)', pct: '7%', detalle: 'Día 70 para directos. Acumulado: 27%.' },
      { hito: 'Bloque 2 (directos: 9 sep 2026 · fondos: ~9 oct 2026)', pct: '7%', detalle: 'Día 90 para directos. Acumulado: 34%.' },
      { hito: 'Bloque 3 (directos: 24 sep 2026 · fondos: ~24 oct 2026)', pct: '7%', detalle: 'Día 105 para directos. Acumulado: 41%.' },
      { hito: 'Bloque 4 (directos: 9 oct 2026 · fondos: ~8 nov 2026)', pct: '7%', detalle: 'Día 120 para directos. Acumulado: 48%.' },
      { hito: 'Bloque 5 (directos: 24 oct 2026 · fondos: ~23 nov 2026)', pct: '7%', detalle: 'Día 135 para directos. Acumulado: 55%.' },
      { hito: '2º cliff — 2º día hábil tras resultados Q3 (directos: ~4 nov · fondos: ~1 dic 2026)', pct: '28%', detalle: 'Acumulado: 83%.' },
  ...
  A: {
    ...
    groups: [
      { label: 'Primera mitad (~50%) — lock-up de 180 días', phases: [
        ...
        { hito: 'Bloque 1 (directos: 20 ago 2026 · fondos: ~24 sep 2026)', pct: '7%', detalle: 'Día 70 para directos.' },
        { hito: 'Bloque 2 (directos: 9 sep 2026 · fondos: ~9 oct 2026)', pct: '7%', detalle: 'Día 90 para directos.' },
        { hito: 'Bloque 3 (directos: 24 sep 2026 · fondos: ~24 oct 2026)', pct: '7%', detalle: 'Día 105 para directos.' },
        { hito: 'Bloque 4 (directos: 9 oct 2026 · fondos: ~8 nov 2026)', pct: '7%', detalle: 'Día 120 para directos.' },
        { hito: 'Bloque 5 (directos: 24 oct 2026 · fondos: ~23 nov 2026)', pct: '7%', detalle: 'Día 135 para directos.' },
        { hito: '2º cliff — 2º día hábil tras resultados Q3 (directos: ~4 nov · fondos: ~1 dic 2026)', pct: '28%', detalle: '' },
...
function buildLp360(positions, investorIds) {
  ...
  if (spxPos.length) {
    ...
    const today = new Date().toISOString().slice(0, 10);
    const hasDir = spxPos.some(p => spxAnchor(p.series?.name || '') === 'dir');
    const hasFon = spxPos.some(p => spxAnchor(p.series?.name || '') === 'fondo');
    const cal = [...(hasDir ? SPX_LOCKUP_B_DIR : []), ...(hasFon ? SPX_LOCKUP_B_FONDO : [])].sort((a, b) => a.date.localeCompare(b.date));
    const nextB = cal.find(e => e.date >= today);
    const blocks = [];
    if (by.B.length) blocks.push({ scope: by.B.join(' · '), summary: 'Liberación escalonada dentro de la ventana de 180 días (directos venden desde el 6 ago 2026; vehículos de fondo desde el 9 sep 2026). Expira por completo el 8 dic 2026.' });
...
function renderInvestorDetail(inv, contacts, positions) {
  ...
  const eventos = (_lp.hasSpx && _lp.lockup) ? `<div class="db-section">
    ...
    <div class="lp-events-note">Estructura del S-1 de SpaceX (IPO 12-jun-2026); fechas del documento oficial de lock-up (29-jul-2026): directos venden desde el 6 ago 2026, vehículos de fondo desde el 9 sep 2026. El prospecto final es la autoridad.</div>
...
const FUND_TRACKERS = {
  fundIV: {
    ...
    active: [
      ...
      { company: 'Cohere',                                      invested: 4868890,  pct: 0.038, mtm: 20284014,  moic: 4.1660, corpVal: 20.5,      pps: 558.25,  entry: 134,     shares: 36335,    fdso: 30.3  },
    ...
    distributed: [
      ...
      { company: 'Groq, Inc. (Distributed)',    invested: 4706172,  pct: 0.037, mtm: 22164552, moic: 4.7097, corpVal: 15.087, pps: 73.0,  entry: 15.5,  shares: 303624, fdso: 206.7 },
    ...
    companyInfo: {
      ...
      'Groq, Inc.':{category:'Semiconductores · IA',stage:'Adquirida',tagline:'Diseña la LPU, chip de inferencia de IA con latencia ultra baja y velocidad líder. En liquidación vía transacción con Nvidia: MVP espera distribuciones totales de al menos $73/acción durante 2026 (~$68.71 ya pagados en 3 distribuciones).',product:{name:'LPU / GroqCloud',desc:'Inferencia de IA de altísima velocidad; acuerdos de gran escala (incl. Nvidia).'},markets:['Semiconductores','Inferencia','Cloud','IA'],thesis:'Ataca el cuello de botella de la inferencia —el costo dominante de la IA en producción.'},
      'Groq, Inc. (Distributed)':{category:'Semiconductores · IA',stage:'Adquirida',tagline:'Misma compañía que Groq (LPU y GroqCloud); esta porción ya fue distribuida a los inversionistas ($48.00 + $4.71 + $16.00 por acción en mar/abr/jul 2026).',product:{name:'LPU / GroqCloud',desc:'Inferencia de IA de altísima velocidad.'},markets:['Semiconductores','Inferencia','Cloud','IA'],thesis:'Porción realizada de la liquidación de Groq vía la transacción con Nvidia.'},
  ...
  fundV: {
    ...
    companyInfo: {
      ...
      'Groq, Inc.': {
        ...
        tagline:'Diseña la LPU, un chip de inferencia de IA con latencia ultra baja y velocidad de generación de tokens líder en la industria. En liquidación vía transacción con Nvidia: MVP espera distribuciones totales de al menos $73/acción durante 2026 (~$68.65 ya pagados en 3 distribuciones).',
...
/* ═══════════════════════════════════════════════════════════════════════════
   ...
   Calendarios/estructuras: SPX_LOCKUP_B_DIR/_FONDO, SPX_LOCKUP_A_EXT, SPX_STRUCTURES (espejo del
...
function spxrEn(s) {
  ...
  const R = [
    ...
    [/Porcentajes del S-1 de SpaceX \(mayo 2026\); fechas del documento oficial de lock-up[^']*/g,
    'Percentages from the SpaceX S-1 (May 2026); dates from the official per-vehicle lock-up document (Jul 29, 2026). The final prospectus is the controlling authority.'],
    ...
    [/Liberación escalonada y ligada a desempeño dentro de la ventana estándar de <b>180 días<\/b>\. Expira por completo ~9 de diciembre de 2026\./g,
     'Staggered, performance-linked release within the standard <b>180-day</b> window. Fully expires ~December 9, 2026.'],
    [/Fecha REAL del doc oficial de lock-up\. Acumulado: 20%\./g,
    'Actual date from the official lock-up document. Cumulative: 20%.'],
    ...
    [/\(directos: /g, '(direct: '],
    [/ · fondos: /g, ' · funds: '],
    [/directos: ~4 nov · fondos: ~1 dic 2026/g, 'direct: ~Nov 4 · funds: ~Dec 1, 2026'],
    [/Día (\d+) para directos\./g, 'Day $1 for direct vehicles.'],
    [/Expiración total del primer tramo\./g, 'Full expiration of the first tranche.'],
    [/Cierre de la primera mitad\./g, 'Close of the first half.'],
